// SVG Particle — Originkit (adapted for Astro: removed "use client")
// @ts-nocheck
...
const EASE = {
    ...
    easeIn: (t) => t * t,
    backOut: (t) => { const c = 1.70158 + 1; return 1 + c * (t - 1) ** 3 + 1.70158 * (t - 1) ** 2 },
...
function mkParticle(src, x, y, idleX, idleY, isExtra = false) {
    return {
        x, y, vx: 0, vy: 0, startX: x, startY: y, repX: 0, repY: 0, repVX: 0, repVY: 0,
        ...
        roamTargetX: 0, roamTargetY: 0, colorIdx: Math.floor(Math.random() * 10),
...
function __OriginkitBase_ParticleImage(__props) {
    const {
        ...
        hoverEnabled = true, hoverConfig = {}, repulsionEnabled = true, repulsionConfig = {},
    ...
    const { repulsionForce = 6, repulsionRadius = 80, repulsionMode = "outside" } = repulsionConfig || {}
    const {
        image: rawImage, mode = "fill", sizeUnit = "%", widthPx = 400, heightPx = 400,
        widthPct = 100, heightPct = 100, scale = 5,
    } = imageConfig || {}
    ...
    physicsRef.current = {
        hover, hoverType, transition, roamWidth, roamHeight, roamOpacity, roamShape, hideType,
    ...
    startAnimRef.current = (newState) => {
        ...
        const { hoverType: ht, roamWidth: rw, roamHeight: rh, roamShape: rs, roamOpacity: rOp, transition: tr } = physicsRef.current
    ...
    const initParticles = () => {
        const {
            image: url, mode: md, sizeUnit: sU, widthPx: wPx, heightPx: hPx,
            widthPct: wPct, heightPct: hPct, scale: sc, particleCount: count,
            hover: hOn, hoverType: ht, roamWidth: rw, roamHeight: rh, roamShape: rs, hideType: hT,
        } = samplingRef.current
        const { W, H } = dimsRef.current
        if (!url || !W || !H) return
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const {
                hover: hOn, hoverType: ht, transition: tr, roamWidth: rw, roamHeight: rh,
                roamOpacity: rOp, roamShape: rs, repulsion: repOn, repulsionForce: rF,
                repulsionRadius: rR, repulsionMode: rMode, particleSize: pSz,
                particleShape: pShape, particleColor: pColor, singleColor: scColor, multiColors: mcColors,
            } = physicsRef.current
...
export default function ParticleImage(props) {
    return <__OriginkitBase_ParticleImage {...props} />
}
