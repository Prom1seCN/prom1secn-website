...
const engines = [
  ...
  { id: 'prom1se', name: 'Prom1seCN', url: '/search?q=', color: '#f97316', icon: <img src="/favicon.png" alt="" style={{ width: 14, height: 14 }} /> },
];
...
export default function SearchBox() {
  ...
  const handleSearch = () => {
    const q = query.trim();
    if (!q) return;
    if (current.id === 'prom1se') {
      window.location.href = current.url + encodeURIComponent(q);
    } else {
